...
**최신 결과·정정은 [카메라별 hidden 비교와 라벨 재검수](CAMERA_UPDATE.md)를 우선합니다.**
시연 검증 10개는 여러 설계 비교에 사용했으므로 손대지 않은 최종 test가 아닙니다.
아래 실제 rollout 표는 오류가 발견된 보존 라벨 기준이며 성공 판정 성능 해석을 보류합니다.
...
episode가 학습과 분리됐더라도 반복적인 설계 비교에서 자유로운 최종 test는 아닙니다.
...
**2026-09-07 정정:** 손목 영상·action 재검수에서 이 경계들이 실제 release보다 늦은 문제가 발견됐습니다.
아래 숫자는 보존 라벨과 맞춘 정도이며, ‘첫 성공을 5.9초 빨리 예측’했다는 해석도 이 잘못된 참조에 종속됩니다.
[검수용 후보와 근거](CAMERA_UPDATE.md#rollout-참조-라벨-정정)는 별도로 보존하며 아직 확정 정답이 아닙니다.
